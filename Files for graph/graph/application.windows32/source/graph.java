...
public void setup() {
  ...
  uno = new Serial(this, uno.list()[PApplet.parseInt(x)], 9600);
  uno.bufferUntil('\n');
...
public void draw(){
  ...
  if(inString != null){
    ...
    for(int i = 0; i < 6; i++){
      ...
      text(Integer.toString(i),(float).01f *width,(float)pre_height - i*pre_height/5);
    }
    //DRAW pinNumber
    ...
    textAlign(RIGHT);
    text(inString + " : " + String.format("%.2f",(float)PApplet.parseFloat(inString)/1024*5) + "v", (float)width*0.9f, vals[scale-1]-10);
    stroke(0);
    //Draw current line
    ...
    vals[vals.length-1] = pre_height - ((float)PApplet.parseFloat(inString)/1024 * pre_height);
    absolute[absolute.length-1] = vals[vals.length-1];
    last = PApplet.parseInt(inString);
    }
    else{
      fill(124);
      noStroke();
      textSize((float)24/200*pre_height);
      textAlign(RIGHT);
      text(Integer.toString(last) + " : " + String.format("%.2f",(float)last/1024*5) + "v", (float)width*0.9f, vals[scale-2]-10);
      stroke(0);
      //Draw current line
